...
read_nom_vqtl <- function(bm, ancestry) {
  if (ancestry == "MA") {
    fread(paste0("../data/processed/vqtl_ss/metal/", bm, "_MA_1.tbl_nom"),
      ...
      separate(SNP, c("CHR", "POS", "ALT", "REF"), sep="[:_]", remove=F)  ## CLARIFY/CONFIRM THE ALLELES FURTHER
  } else {
...
Using the set of 20 adjusted biomarkers in the European sample, this produced an estimate of 16.2 effective biomarkers, for a corrected p-value threshold of 5$\times$10^-8^ / 16.2 = 3.1$\times$10^-9^.
...
alt_p2 <- ggplot() +
  geom_point(data=filter(alt_mh_data, source == "me"), 
             aes(x=x_coord, y=nlp, color=factor(color)), 
             size=0.75, alpha=1) +
  geom_hline(yintercept=-log10(bonferroni_thresh), linetype="dashed", color="black") + 
  scale_x_continuous(breaks=lims$avg_coord[c(1:16, 18, 20, 20, 22)], 
                     labels=c(1:16, 18, 20, 20, 22), 
                     expand=c(0,0)) +
  # scale_y_continuous(name=expression(-log[10](italic(p)))) +
...
```{r vqtl-selection}
vqtl_var_df <- bind_rows(
  map(pruned_vqtl_ss, bind_rows, .id="ancestry"), .id="bm"
) %>%
  select(index_var, CHR, POS, REF, ALT, bm, ancestry, P)
...
The `r nrow(distinct(filter(vqtl_var_df, ancestry == "MA", P < bonferroni), index_var))` variants passing a multiple testing-corrected genome-wide threshold (p = `r signif(bonferroni, 2)`) will be taken forward for the formal analysis. However, all `r nrow(distinct(filter(vqtl_var_df, P < bonferroni), index_var))` index variants passing 5e-8 in the meta-analysis or any ancestry-specific analysis will be tested to have these results available.
