...
batch <- data.table::fread("/humgen/florezlab/UKBB_app27892/ukb27892_cal_chrAUT_v2_s488363.fam", 
			   header=F, col.names=c("FID", "IID", "P1", "P2", "S", "Batch")) %>% 
  select(IID, Batch)
...
base_phenos <- read.table("/humgen/florezlab/users/jcole/UKBB/diet/vqtl/pheno_processing/src/covariates/merged.phe", header=T, stringsAsFactors=F) %>%
    ...
    left_join(batch, by="IID") %>% 
    left_join(eth, by="IID")
...
sas_phenos %>% mutate(id=IID) %>% write_csv("/humgen/florezlab/users/jcole/UKBB/diet/vqtl/pheno_processing/data/vqtl_phenos_SAS_modelrivas2.csv")
