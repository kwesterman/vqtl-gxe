...
vqtl_list <- lapply(setNames(all_biomarkers, all_biomarkers), function(bm) {
...
names(vqtl_list) <- all_biomarkers

me_list <- lapply(setNames(all_biomarkers, all_biomarkers), function(bm) {
  fread(paste0("../data/processed/main_effect_ss/metal/", bm, "_MA_1.tbl_nom"),
        stringsAsFactors=F, data.table=F) %>%
    select(SNP, P, Dir, ISq, HetP) %>%
    separate(SNP, c("CHR", "POS", "ALT", "REF"), sep="[:_]", remove=F)  ## CLARIFY/CONFIRM THE ALLELES FURTHER
})
names(me_list) <- all_biomarkers
```
...
```{r manhattan-plots}
vqtl_mh_plts <- lapply(setNames(all_biomarkers, all_biomarkers), function(bm) {
  make_manhattan(vqtl_list[[bm]], "P")
})

me_mh_plts <- lapply(setNames(all_biomarkers, all_biomarkers), function(bm) {
...
vqtl_me_overlaps <- lapply(setNames(all_biomarkers, all_biomarkers), function(bm) {
